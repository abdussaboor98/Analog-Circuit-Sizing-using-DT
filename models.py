...
class ActorCriticMLP:
    class Actor(torch.nn.Module):
        ...
        def forward(self, state):
            ...
            actions = torch.tensor((), dtype=torch.float32).to(device)
            for i in range(batch_size):
                x = state[i].float()  # Ensure float32
    ...
    class Critic(torch.nn.Module):
        ...
        def forward(self, state, action):
            ...
            action = action.repeat_interleave(self.num_nodes, 0).reshape(
                batch_size, self.num_nodes, -1)
            data = torch.cat((state.float(), action.float()), axis=2)  # Ensure float32
    
            values = torch.tensor((), dtype=torch.float32).to(device)
            for i in range(batch_size):
                x = data[i]
                x = F.relu(self.mlp1(x))
                x = F.relu(self.mlp2(x))
